...
function HttpClient(operation, params)
{
	...
	function init()
	{
		//need to specify whole list here params.url.length
		var num = Math.floor(Math.random() * numUrls);
		options = urlLib.parse(params.url[num]);
		options.headers = {};
		if (params.headers)
		{
	...
	function getRequestFinisher(id)
	{
		return function(error, result)
		{
			...
			operation.callback(error, result, callback, options.href);
		};
	}
...
var Operation = function(options, Constructor, callback)
{
	 ...
	 */
	self.callback = function(error, result, next, url)
	{
		requests += 1;
		...
		if (options.statusCallback) {
			options.statusCallback(self.latency.getResults(), result, error, url);
		}
	};
